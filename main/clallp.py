...
# Load the TTL file
g = Graph()
g.parse("coreclass.ttl", format="ttl", encoding='utf-8')
...
# Define namespaces based on your TTL file
...
# Bind prefixes based on your TTL file
g.bind('rdf', RDF_ns)
g.bind('rdfs', RDFS_ns)
g.bind('owl', OWL_ns)
g.bind('bfo', BFO_ns)
...
# Prepare CSV file
with open('output_core_classes6.csv', 'w', newline='', encoding='utf-8-sig') as csvfile:
    fieldnames = ['class_uri', 'class_label'] + [name for uri, name in properties_to_extract if name != 'rdfs_label']
    ...
    for cls in classes:
        ...
        for label in g.objects(cls, RDFS.label):
            labels.add(clean_literal(label))
        if labels:
            row['class_label'] = '; '.join(labels)
        else:
            # Use hardcoded mapping if label is missing
            if cls in bfo_number_to_label:
                row['class_label'] = bfo_number_to_label[cls]
            else:
                row['class_label'] = ''
