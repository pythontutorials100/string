...
# --- Helper Functions (clean_uri, safe_float_convert) ---
def clean_uri(uri):
...
# --- get_defect_info (SPARQL Query) - Remains the same as previous correct version ---
def get_defect_info(target_defect_id: str, endpoint: str, auth: tuple = None) -> pd.DataFrame | None:
    ...
    try:
        ...
        for result in bindings:
            row = { "Defect": clean_uri(result.get("defect", {}).get("value")), "Defect ID": result.get("defectID", {}).get("value", ""), "Airfoil": clean_uri(result.get("airfoil", {}).get("value")), "Airfoil Part Num": result.get("airfoilPartNum", {}).get("value", ""), "Defect Region": clean_uri(result.get("defectRegion", {}).get("value")), "Region Part": clean_uri(result.get("regionPart", {}).get("value", "")), "Index1 Val": result.get("index1Val", {}).get("value", ""), "Index1 Datum Part": clean_uri(result.get("index1DatumPart", {}).get("value")), "Index1 Datum Part Type": clean_uri(result.get("index1DatumPartType", {}).get("value")), "Index2 Val": result.get("index2Val", {}).get("value", ""), "Index2 Datum Part": clean_uri(result.get("index2DatumPart", {}).get("value")), "Depth Val": result.get("depthVal", {}).get("value", ""), "Length Val": result.get("lengthVal", {}).get("value", ""), "Blend Process": clean_uri(result.get("blendProc", {}).get("value", "")), "Blend Participant": clean_uri(result.get("blendParticipant", {}).get("value", "")), "Blend Depth": result.get("blendDepth", {}).get("value", ""), "Blend Length": result.get("blendLength", {}).get("value", ""), "Blend Flat Len": result.get("blendFlatLen", {}).get("value", ""), "Blend Fillet": result.get("blendFillet", {}).get("value", ""), "Blend Prop Factor": result.get("blendPropFactor", {}).get("value", ""), "Blend Loc": result.get("blendLoc", {}).get("value", ""), "Blend Node Comp": result.get("blendNodeComp", {}).get("value", "") }
            data.append(row)
...
class MainWindow(QtWidgets.QMainWindow):
    # --- __init__ method (UI setup) remains the same ---
    def __init__(self, parent=None):
        super().__init__(parent); self.setWindowTitle("Airfoil Defect Visualization"); self.setGeometry(100, 100, 1400, 900)
        self.defect_data = None; self.defect_actors = []; self._initialize_defect_params()
    ...
    def _initialize_defect_params(self):
        ...
        self.datum_part = "N/A"; self.datum_part_type_name = "N/A"; self.datum_type = "Unknown"
        ...
        self.pos_red1, self.pos_red2, self.pos_red3 = [pos_top_front] * 3
        self.short_label = "Index 1: N/A in"; self.far_label = "Index 2: N/A in"; self.depth_label = "Depth: N/A in"
        self.arrow_depth_start, self.arrow_depth_end = [pos_top_front] * 2; self.line1_start, self.line1_end = [pos_top_front] * 2
        self.line2_start, self.line2_end = [pos_top_front] * 2; self.txt_pos_v1, self.txt_pos_v2, self.txt_pos_depth = [pos_top_front] * 3

    # <<< MODIFIED load_and_update_defect with DEBUG PRINTS >>>
    def load_and_update_defect(self):
        """Fetches defect data, determines datum type FROM DATUM PART TYPE, and updates visualization."""
        ...
        try:
            ...
            if defect_df is None: QMessageBox.critical(self, "Query Error", "Failed to query GraphDB."); self._initialize_defect_params()
                ...
                self.datum_part = self.defect_data.get("Index1 Datum Part", "N/A")
                self.datum_part_type_name = self.defect_data.get("Index1 Datum Part Type", "N/A") # <-- Get TYPE name
                # <<< --- ADDED DEBUG PRINT --- >>>
                print(f"DEBUG: Fetched Datum Part='{self.datum_part}', Type Name='{self.datum_part_type_name}'")
                # <<< --- END DEBUG PRINT --- >>>
                ...
                # --- Determine Datum Type BASED ON TYPE NAME ---
                # Define keywords to check for edge types (lowercase)
                edge_keywords = ["edge", "le", "te", "leading", "trailing"] # Added more specific words
                # Use datum_part_type_name for logic
                type_name_lower = self.datum_part_type_name.lower() if self.datum_part_type_name else ""

                if type_name_lower and "tip" in type_name_lower:
                    self.datum_type = "Tip"
                elif type_name_lower and any(edge_kw in type_name_lower for edge_kw in edge_keywords):
                    self.datum_type = "Edge"
                else:
                    # Default to Tip if type is missing or unrecognized
                    self.datum_type = "Tip"
                    print(f"Warning: Could not determine datum category from part type '{self.datum_part_type_name}'. Defaulting to Tip.")

                # <<< --- ADDED DEBUG PRINT --- >>>
                print(f"DEBUG: Determined Datum Type: '{self.datum_type}'")
                # <<< --- END DEBUG PRINT --- >>>
    ...
    # --- _calculate_visualization_data remains the same ---
    def _calculate_visualization_data(self):
        datum_origin = pos_top_front
        if self.datum_type == "Edge": # Check the determined type
            print("Calculating for EDGE datum.")
            vec_direction = pos_top_back - datum_origin
            depth_vector = np.array([0.0, 1.0, 0.0]) # Example: Depth along +Y
        else: # Tip or Unknown (defaults to Tip)
            if self.datum_type != "Tip": print("Defaulting to TIP datum calculation.") # Should only happen if forced to Tip above
            vec_direction = pos_bottom_front - datum_origin
            depth_vector = np.array([1.0, 0.0, 0.0]) # Depth along +X
        norm_vec_direction = np.linalg.norm(vec_direction)
        if norm_vec_direction < 1e-6: unit_vec_direction = np.array([0.0, -1.0, 0.0]) # Default fallback
        else: unit_vec_direction = vec_direction / norm_vec_direction
        ...
        self.short_label = f"Index 1 = {self.indexVal1_input:.3f} in"; self.far_label = f"Index 2 = {self.indexVal2_input:.3f} in"; self.depth_label = f"Depth = {self.depthVal_input:.3f} in"
        self.arrow_depth_start = mid_point_on_vec; self.arrow_depth_end = self.pos_red3
        offset_1 = np.array([-5.0, 0.0, 0.0]); offset_2 = np.array([-10.0, 0.0, 0.0])
        self.line1_start = self.pos_red1 + offset_1; self.line1_end = datum_origin + offset_1
        self.line2_start = self.pos_red2 + offset_2; self.line2_end = datum_origin + offset_2
        self.txt_pos_v1 = (self.line1_start + self.line1_end) / 2; self.txt_pos_v2 = (self.line2_start + self.line2_end) / 2; self.txt_pos_depth = (self.arrow_depth_start + self.arrow_depth_end) / 2

    # --- _setup_base_plot remains the same ---
    def _setup_base_plot(self):
        self.plotter.clear(); self.plotter.enable_anti_aliasing('fxaa')
        try: self.airfoil_mesh = pv.read(stl_file); self.plotter.add_mesh(self.airfoil_mesh, color="lightgray", opacity=1.0)
        except FileNotFoundError: QMessageBox.critical(self, "File Error", f"Cannot find STL file: {stl_file}"); self.airfoil_mesh = pv.Sphere(); self.plotter.add_mesh(self.airfoil_mesh, color="red")
    ...
    def _update_plot(self):
        ...
        print(f"Rendering plot. {len(self.defect_actors)} defect actors present (Datum Type: {self.datum_type}).")
        self.plotter.render()

    # --- _update_index_depth_table remains the same ---
    def _update_index_depth_table(self):
        ...
        data = { "Datum Part": self.datum_part, "Datum Part Type": self.datum_part_type_name, "Detected Datum": self.datum_type, "Index 1": f"{self.indexVal1_input:.3f}", "Index 2": f"{self.indexVal2_input:.3f}", "Depth": f"{self.depthVal_input:.3f}" }
        table.setColumnCount(2); table.setRowCount(len(data)); table.setHorizontalHeaderLabels(["Location Parameter", "Value"])
        ...
        for key, value in data.items():
            param_item = QtWidgets.QTableWidgetItem(key); value_text = f"{value} in" if key in ["Index 1", "Index 2", "Depth"] else str(value)
    ...
    def _update_display(self): print("Updating display..."); self._calculate_visualization_data(); self._update_plot(); self._update_index_depth_table(); self._update_blend_table(); print("Display update complete.")
