...
def generate_triples_for_row(row: dict) -> list:
    """
    Given one row of extended CSV data, return a list of "Subject,SubjectType,Predicate,Object,ObjectType" lines
    representing the instance data (ABox).
    """
    triples = []
    ...
    if (fillet_radius_str or blend_depth_str or blend_length_str or
        ...
        # Fillet radius
        # (similar; uses generate_measurement_triples)
        if fillet_radius_str:
            # If fillet radius is numeric
            triples += generate_measurement_triples(
        ...
        # Proportional factor (often dimensionless ratio)
        # We'll do a custom snippet for that:
        if prop_factor_str:
...
def main():
    ...
    with open(INPUT_CSV, "r", newline="", encoding="utf-8") as f_in, \
        ...
        # 2) Write TBox lines
        # Each line is "Subject,SubjectType,Predicate,Object,ObjectType"
        # We can just split the string by commas. 
        ...
        for row in reader:
            ...
            for triple_str in row_triples:
                # triple_str also has 5 parts separated by commas
                # but to be safe, let's split and validate or re-join
